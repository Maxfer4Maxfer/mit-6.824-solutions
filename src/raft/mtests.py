...
class StatsMeter:
    ...
    count, mean, variance
    Uses Welford's algorithm to compute sample mean and sample variance
    incrementally.
...
shortcut = {
    ...
    '2B': [
        "TestBasicAgree2B",
        "TestRPCBytes2B",
        "TestFor2023TestFollowerFailure2B",
        "TestFor2023TestLeaderFailure2B",
        "TestFailAgree2B",
        "TestFailNoAgree2B",
...
def run_tests(
        ...
        workers: int = typer.Option(
            8, '--workers', '-p', help='Number of parallel tasks'),
        iterations: int = typer.Option(
            10, '--iter', '-n', help='Number of iterations to run'),
        timeout: str = typer.Option(
            "30s", '--timeout', '-t', help='Timeout for each test'),
        output: Optional[Path] = typer.Option(
            None, '--output', '-o', help='Output path to use'),
        output_base_dir: Optional[Path] = typer.Option(
            "logs", '--output-dir', '-d', help='Base output dir to use'),
        verbose: int = typer.Option(
            0, '--verbose', '-v', help='Verbosity level', count=True),
        archive: bool = typer.Option(
            False, '--archive', '-a',
            help='Save all logs intead of only failed ones'),
        race: bool = typer.Option(
            False, '--race/--no-race', '-r/-R', help='Run with race checker'),
        coverage: bool = typer.Option(
            False, '--coverage', '-c',
            help='Output path for a total caverage raport'),
        coverage_output: Optional[Path] = typer.Option(
            None, '--coverage-output', '-co',
            help='Special output for a total caverage report'),
        loop: bool = typer.Option(
            False, '--loop', '-l', help='Run continuously'),
    ...
    while True:
        ...
        if sequential:
            test_instances = itertools.chain.from_iterable(itertools.repeat(
                                                               test,
                                                                iterations)
                                                           for test in tests)
        else:
