...
def main(
        ...
        cleck: bool = typer.Option(
            False, '--cleck/--no-cleck', '-clrk',
            help='Show cleck log'),):
    topics = list(TOPICS)

    # debug
    file = open(
        "/home/maxim/go/src/github.com/maxfer4maxfer/mit-6.824/src/kvraft/log.txt",
    )
    n_columns = 5
    cleck = True
    ...
    for line in input_:
        try:
            ...
            # S5 ELECT_12345 18:19:20.292541 raft.go:360: 6 peer voted against us
            peer, topic_cID, time, code_line, *msg = line.strip().split(" ")

            if peer != "CK" and len(peer) != 2:
                console.print(line, end="")
            ...
            # space. Heavylifting is done through rich.column.Columns object
                ...
                if cleck and peer == "CK":
                    i = 0
                if cleck and peer != "CK":
                    i = int(peer[1]) + 1 # peer = "R<N>"
                if not cleck:
                    i = int(peer[1])  # peer = "R<N>"
